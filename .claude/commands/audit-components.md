...
- All `.tsx` files under `src/app/` and `src/components/`
- Excludes `src/components/uui/**` (vendor library code)
- Excludes `src/lib/`, `src/hooks/`, `src/utils/`, `src/types/` (not components)
...
- Exclude `src/components/shared/section-label.tsx` itself and any CSS files
- Each match is a FAIL — should use `<SectionLabel>` instead

**ScrollReveal bypass:**
- Grep pattern: `useInView` in component files under `src/components/` (not hooks/)
...
Each match is a WARN. If `src/lib/constants.ts` exists and exports these values, downgrade matching files that import from it to PASS.
...
**`any` type usage:**
- Grep pattern: `: any` or `as any` in `.tsx` files under `src/` (exclude `uui/`)
