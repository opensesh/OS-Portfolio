...
export default function FaultyTerminal({
  ...
  // Listen on window for mouse tracking — the container sits behind other layers
  ...
  // Update uniforms reactively without recreating the WebGL context
  useEffect(() => {
    ...
    program.uniforms.uBackground.value = new Color(bgVec[0], bgVec[1], bgVec[2]);
    ...
    // Update clear color to match background
    const renderer = rendererRef.current;
    if (renderer) {
      renderer.gl.clearColor(bgVec[0], bgVec[1], bgVec[2], 1);
    }
  }, [
    scale, gridMul, digitSize, scanlineIntensity, glitchAmount,
    flickerAmount, noiseAmp, chromaticAberration, ditherValue,
    curvature, tintVec, bgVec, mouseStrength, mouseReact, brightness,
  ]);
  ...
  useEffect(() => {
    ...
    // Only recreate context when DPR changes — all other uniforms update reactively
