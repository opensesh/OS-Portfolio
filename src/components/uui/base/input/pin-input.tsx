...
import { Label as LabelBase } from "@/components/uui/base/input/label";
...
const styles = {
    xxxs: { group: "gap-1.5 h-9", slot: "size-9 px-3 py-2 text-sm rounded-none font-medium text-placeholder/50", caret: "text-sm font-medium" },
    xxs: { group: "gap-2 h-10", slot: "size-10 px-3 py-2 text-md rounded-none font-medium text-placeholder/50", caret: "text-md font-medium" },
    xs: { group: "gap-2 h-11", slot: "size-11 px-3.5 py-2.5 text-md rounded-none font-medium text-placeholder/50", caret: "text-md font-medium" },
...
const Slot = ({ index, className, ...props }: ComponentPropsWithRef<"div"> & { index: number }) => {
    ...
    return (
        <div
            ...
            className={cx(
                "relative flex items-center justify-center rounded-none bg-primary text-center text-placeholder/40 shadow-xs ring-1 ring-primary transition-[box-shadow,background-color] duration-100 ease-linear ring-inset",
                styles[size].slot,
